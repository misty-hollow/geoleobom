...
갱신: 2026-09-15 (Week 4 구현 진행 — 지도 중심 검색·경로 UX 병합, 세션 장소명 Draft PR). 운영 변화·주간 정리·막힘·중단/인계 때 갱신한다.
...
- **현재 작업: Week 4 구현 (2026-09-15, Opus 5).** 규약 개정(아래 2026-09-14 기록)은 끝났고 지금은 구현 단계다.
  ...
  - **세션 장소명(DESIGN.md §23) — Draft PR 진행 중.** 장소 표기(`name`·`source`)를 라우트보다 위(`App`)의 메모리 보관함으로 올리고, 결과 헤더·담은 후보 목록·비교 헤더가 **같은 resolver**를 쓰게 했다. 저장 계층은 그대로다 — 후보는 계속 좌표만 최대 4개이고 이름·출처는 localStorage·sessionStorage·URL·`history.state` 어디에도 넣지 않는다. 새로고침하면 표기가 사라지고 좌표로 돌아간다.
  - **두 병합분 모두 아직 production에 배포하지 않았다.** 운영은 여전히 `b070bcb`(Week 3 배포본)다.
  - **아직 구현되지 않은 Week 4 항목:** 현위치 입력(§24), 비교표 재설계(§14). 현위치는 이미 승인된 기능이며 Week 7까지 착수한다.
  - **정하지 않은 것:** 비교 열 간 `versions` 불일치의 재분석 여부(v2.5 부록 G).
