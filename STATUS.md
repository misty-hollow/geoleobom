...
- **다음 작업: 데이터 파이프라인 → 실제 OSRM 검증 → adapter 연결.** Docker Desktop이 준비돼 세 단계를 모두 진행할 수 있다. 착수 전 결정이 필요한 항목은 OSRM 이미지 레지스트리·태그 고정과 osmium 실행 방법이다.
...
- **Docker Desktop 설치 완료 (2026-09-11)** — Engine 29.7.2, Compose v5.5.1, context `desktop-linux`, 12 CPU·약 15.4GB. 별도 Ubuntu 배포판 없음. 다만 `docker` 실행 파일이 **PATH에 없어** 스크립트는 전체 경로를 쓰거나 사용자가 PATH를 등록해야 한다.
- **게이트 1의 OSRM 이미지 태그 고정은 아직 미완료.** Docker Hub `osrm/osrm-backend`는 v5.25.0(2021)에서 멈췄고 공식 최신은 GHCR에 있다. 레지스트리·태그 선택은 착수 전 결정 사항이다.
