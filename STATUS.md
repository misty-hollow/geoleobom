...
- **현재 작업: Week 4 UI/UX 재설계 — 규약 개정 단계 완료, 구현 전 (2026-09-14, Fable 5.1).**
  ...
  - **다음 행동(Engineering, Opus):** v2.5 4-4 `/api/search` 지도 중심 계약 구현(카카오 bias 파라미터·두 갈래 합치기·로그 제외 검사) → DESIGN.md §7·§9·§11·§14·§22~§24 순으로 프론트 구현 → 현위치는 Week 7까지 착수 결정. 5-8 Engineering 확인 항목(프로그램 이동 이벤트 가드, `getLevel()` 조건, tfoot 임계 640 실기기, 200% 확대, 경로 전환 스크린리더 알림)은 구현 PR에서 닫는다.
