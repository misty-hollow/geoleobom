...
갱신: 2026-09-13 (Week 3 배포 시도 중단·복구, web release 지문 이식성 수정). 운영 변화·주간 정리·막힘·중단/인계 때 갱신한다.
...
- **현재 작업: web 릴리스 지문의 OS 의존성 수정 (2026-09-13, Opus 5). 브랜치 `fix/web-release-digest-portable`, base `3ed6f924`.**
  ...
  - **고친 것:** 지문이 **상대 경로와 그 파일 내용의 SHA-256, 그 둘만** 담는다. `sha256sum`에서 해시 값만 꺼내고(`cut -c1-64`) 경로는 NUL로 감싸 직접 스트림에 넣는다. 표시 서식·바이너리 표시(`*`)·OS별 공백·로캘은 지문에 들어가지 않는다. 경로가 남으므로 이름 변경도 여전히 잡는다.
