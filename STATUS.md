...
- **게이트 2의 계산 정확성 항목도 실제 배포본에서 확인됐다.** 목적지 160개 요청 40건, 추가 배치 26건. `capped`·`complete` 두 경로 모두 실동작.
...
| 계산 정확성 | 캐시 분리 (같은 4자리 격자, 다른 5자리) | 완료 — 단위 검사. 부하에서도 좌표 5번째 자리를 바꿔 대부분 미스로 돌았다(서버 로그 기준 miss 60·hit 5) |
