...
갱신: 2026-09-14 (v2.5 규약 개정 — Week 4 제품 결정 2건 반영, 코드 미변경). 운영 변화·주간 정리·막힘·중단/인계 때 갱신한다.
...
- **현재 작업: Week 4 UI/UX 재설계 — 규약 개정 단계 완료, 구현 전 (2026-09-14, Fable 5.1).**
  ...
  - **다음 행동(Engineering, Opus):** v2.5 4-4 `/api/search` 지도 중심 계약 구현(카카오 bias 파라미터·두 갈래 합치기·로그 제외 검사) → DESIGN.md §7·§9·§11·§14·§22~§24 순으로 프론트 구현. 현위치(§24)는 **이미 승인된 기능**이며 구현 시점만 남았다 — Week 8 신규 핵심 기능 금지 전인 Week 7까지 착수한다. 5-8 Engineering 확인 항목(프로그램 이동 이벤트 가드, `getLevel()` 조건, tfoot 임계 640 실기기, 200% 확대, 경로 전환 스크린리더 알림)은 구현 PR에서 닫는다.
  - **정하지 않은 것:** 비교 열 간 `versions` 불일치의 재분석 여부(v2.5 부록 G). 카카오맵 도보 길찾기 바로가기·Kakao 도보 Routing API는 철회.
