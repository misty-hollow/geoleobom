...
- **현재 작업: Pre-Week3 hardening — 완료 (B급).** PR #16 병합(`52d5bd7`), 운영 배포·스모크·게이트 2 재측정까지 끝냈다. Fable 5.1 독립 검토 2회(1차 차단 1건, delta merge). GPT Astra의 누적 감사가 "다음 단위 전에 수정 필요"로 판정한 계산·동시성·배포·로그 방어 공백을 고친다. 8건 중 6건은 현재 main에서 재현했고, 1건(README/validate_gpkg 설명 불일치)은 코드로 확인했으며, **1건(OSRM `/nearest` NoSegment → 502)은 반례가 재현되지 않았다** — 아래 "막힌 점"을 본다.
- **다음 작업: Week 3 프론트.** 검색·지도 핀·결과 화면. `/api/search`(카카오 프록시)와 `/api/route`가 501로 남아 있고 게이트 2의 응답시간 항목이 그 둘을 요구한다.
...
| 성능·자원 | 한국 내 클라이언트 응답시간 실측·기록 | **부분** — `/api/analyze`만 유선 실측(동시 4요청 체감 중앙 808ms). 10절은 `/api/search`와 **검색→분석→경로 표시 흐름**까지 요구하는데 `/api/search`·`/api/route`가 아직 501이다. 모바일(LTE)도 미측정 |
