...
- **운영 서버가 실데이터로 동작한다.** `https://geoleobom.kr/api/analyze`가 공식 원본 3종으로 만든 배포본 `2026Q3-cc-01`(123,963행)으로 답한다. caddy·osrm·api 세 컨테이너가 떠 있다.
...
- **5좌표 스모크 통과.** 기준값은 배포본마다 있다 — `deploy/smoke_baseline/2026Q3-cc-01.json`(실데이터), `synthetic-cc-01.json`(합성).
...
- **후보 제한(20개) 누락을 실데이터로 측정했다.** 5지점 × 5항목 중 **8건은 제한이 최근접 1등을 바꿀 수 있다**(보행거리 ≤ 직선×2 가정). v2.3 3절이 이미 고지한 한계이며 이제 수치가 있다.
...
| 계산 정확성 | 픽스처 5좌표 기대값 기록 | 완료 — `deploy/smoke_baseline/2026Q3-cc-01.json`(실데이터)·`synthetic-cc-01.json` |
| GIS·후보 품질 | R*Tree 후보 조회 vs 전수 대조 | 완료 (2026-09-11) |
| GIS·후보 품질 | 공주 보행망 데스크체크 15~20경로 ≥80% | **미측정 (B 담당)** |
| GIS·후보 품질 | 대표 5지점 후보 제한(20개) 누락 측정 | **완료 (2026-09-11 실데이터)** — `data.gate2_quality`. 25개 조합 중 8건은 제한이 1등을 바꿀 수 있다(보행 ≤ 직선×2 가정) |
