...
갱신: 2026-09-15 (Week 4 구현 진행 — 지도 중심 검색 병합, 경로 UX Draft PR). 운영 변화·주간 정리·막힘·중단/인계 때 갱신한다.
...
- **현재 작업: Week 4 구현 (2026-09-15, Opus 5).** 규약 개정(아래 2026-09-14 기록)은 끝났고 지금은 구현 단계다.
  - **지도 중심 기준 검색 B안 — main에 병합됐다.** PR #23, merge `188df60e313d5f850af7a7fa2a4fa729554c8863`. `/api/search`가 선택 `lon`·`lat`를 받고(둘 다 또는 둘 다 없음, 한쪽만이면 422), 키워드 갈래에만 카카오 `x`·`y` bias를 실으며(`radius`·`sort`는 보내지 않는다 — 먼 정확 일치를 잘라내므로), 주소 갈래는 전국 그대로다. 응답 필드는 넷 그대로이고 결과 행 거리는 프론트가 계산한다. 실제 카카오 호출로 확인했다(대전 중심 `공주대` → 신관 25.5km 1위, 천안 59.4km 2위로 보존). **운영 배포는 하지 않았다** — 운영은 여전히 `b070bcb`다.
  - **경로 UX(DESIGN.md §7·§11) — Draft PR 진행 중.** 경로 fit 상태 머신(첫 표시 fit / 전환 시 자동 확대 없음 / `userMoved` 분기 / 프로그램 이동 이벤트 가드), 목적지 16px 링, RoutePanel(태그·12px 링·× 아이콘 44×44 `aria-label`), 배치 전환·스냅 변화·경로 제거에서 중심·배율 유지. **실제 카카오 SDK + 실 OSRM QA(`npm run qa:kakao`)는 아직 돌리지 못했다** — 이 PC에서 Docker 엔진이 내려가 있어 실 OSRM을 띄울 수 없었다.
  - **아직 구현되지 않은 Week 4 항목:** 현위치 입력(§24), 세션 장소명(§23), 비교표 재설계(§14). 현위치는 이미 승인된 기능이며 Week 7까지 착수한다.
