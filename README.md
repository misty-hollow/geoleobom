...
# 검색 -> 분석 -> 경로 흐름의 체감·서버 내부 시간을 나눠 잰다 (10절)
python deploy/measure_flow.py --base-url https://geoleobom.kr --host geoleobom \
    --query "공주대학교" --rounds 5 --network "유선" --location "한국"
```
...
### 게이트 2 한국 내 응답시간 실측 (2026-09-13, 운영 배포본 `b070bcb`)

10절의 마지막 부분 항목을 닫은 측정이다. 측정 위치는 **개발 PC(대한민국, 유선)**,
서버는 Los Angeles다. 이전까지 이 항목이 "부분"이었던 이유는 `/api/search`·`/api/route`와
화면이 운영에 없어서였다. Week 3 프론트가 배포된 뒤 같은 방법으로 쟀다.
...
5라운드 **5/5 성공**(실패 라운드 0). 라운드마다 좌표 5번째 자리를 옮겨 캐시 미스를 만든다.

| 단계 | 클라이언트 체감(중앙) | 서버 내부(중앙) | 차이 |
|---|---|---|---|
| `/api/search` | 634.5 ms | 559.6 ms | 75 ms |
| `/api/analyze` | 431.0 ms | 5.5 ms | 425 ms |
| `/api/route` | 438.1 ms | 11.3 ms | 427 ms |
| 흐름(세 단계 합) | **1,496.7 ms** (최소 1,472.4 · 최대 2,416.8) | — | — |

**차이가 곧 리전 지연이다.** `/api/analyze`는 서버 안에서 5.5ms인데 체감은 431ms다 —
계산이 아니라 태평양 왕복이 시간을 쓴다. `/api/search`만 서버 내부가 559.6ms로 큰데,
그 단계는 서버가 카카오 REST를 다시 부르므로 상류 왕복이 서버 내부 시간에 들어간다.
`/api/search`는 첫 라운드만 체감 1,510ms였고(TLS·연결 수립) 나머지 네 라운드는 627~643ms로 모였다.

동시 4요청도 같은 배포본에서 다시 쟀다 — 이전 값은 `/api/analyze`만 있던 때의 것이다.

| 항목 | 기준 | 측정값 | 판정 |
|---|---|---|---|
| 동시 4요청 × 20건 (체감) | 오류 없음 | 최소 529 · 중앙 967 · p95 1,562 · 최대 1,734 ms, **실패 0** | 통과 |
| 그 20건의 캐시 상태 | — | **서로 다른 캐시 키 20 / 20** (전부 미스) | 기록 |
| 부하 중 `MemAvailable` 최솟값 | 약 1GB 이상 | **2,774 MB** (표본 144개) | 통과 |
| 부하 중 스왑 | 지속적 I/O 없음 | 사용 0 MB, `pswpin`/`pswpout` 증가 **0 페이지** | 통과 |
| 접근 로그의 좌표·쿼리 문자열·검색어·헤더·REST 키 | 0건 | **0건** | 통과 |

**이 값이 무엇이 아닌가.** 흐름은 HTTP 세 번의 왕복 합이다. **검색 결과를 고른 뒤 화면에
경로가 그려질 때까지의 시간이 아니다** — 카카오 SDK 내려받기, 번들 파싱, 지도 타일 수신,
경로선 렌더, 시트 애니메이션은 어느 것도 여기 없다. 화면 쪽은 브라우저로 따로 확인했고
(390×844·1280×800에서 타일 로드·검색·결과·경로·직접 `/p` 진입, console error 0,
asset 404 0) 그것은 **시간 측정이 아니라 배포 확인**이다.

**모바일(LTE)은 여전히 미측정이다.** 이번 실측은 유선뿐이라 이 항목의 완료는 유선 기준이다.
iOS·Android 실기기와 장시간 운영 안정성도 미검증이다.

**로그 privacy는 새 Caddy가 적용된 상태에서 봤다.** 좌표가 든 `/p` 진입과 검색을 실제로
발생시킨 뒤 접근 로그의 `uri`에 남은 값은 템플릿뿐이었다 — `/`, `/assets`, `/api/analyze`,
`/api/route`, `/api/search`, `/api/health`, `/p`, `/favicon.svg`. `remote_ip`는 /24로
마스킹된다(공개 트래픽 100줄 전부 끝자리 0). 헤더가 남은 2줄은 Caddy 자신의 admin
...
**응답시간 항목은 아직 "완료"가 아니다.**(→ 2026-09-13에 닫혔다. 위 "게이트 2 한국 내 응답시간 실측"을 본다. 아래는 그때의 기록이다.) 10절은 `/api/analyze`뿐 아니라 **`/api/search`와 검색→분석→경로 표시 흐름**의 응답시간도 요구한다. `/api/search`·`/api/route`와 화면이 생겼으므로 이제 잴 대상은 있지만, **운영 서버에서 그 흐름을 아직 측정하지 않았다**(`deploy/measure_flow.py`가 그 측정을 담당한다). **모바일(LTE)도 측정하지 않았다** — 유선만 쟀다.
