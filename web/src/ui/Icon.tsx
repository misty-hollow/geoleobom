...
const PATHS: Record<IconName, React.ReactNode> = {
  ...
  /*
   * 현위치 (DESIGN.md 6절 M8): **눈금(4방향 tick)이 있는 크로스헤어.**
   *
   * 목적지 링(7-1)은 눈금 없는 매끈한 링 + 가운데 점이다. 6절이 두 형태를 닮게 만들지
   * 말라고 한 이유가 "링 = 내 위치"로 읽히면 출발·도착이 뒤집히기 때문이라, 여기에는
   * **가운데 점을 넣지 않는다** — 눈금만으로 구분된다.
   */
  locate: (
    <>
  ...
  /*
   * 공주대로 돌아가기. **현위치와 같은 글리프를 쓸 수 없다** — 지원 지역 밖에서 두
   * 버튼이 컨트롤 스택에 나란히 선다(24절: 현위치 첫 자리, `공주대로 돌아가기` 둘째).
   * 6절이 글리프를 지정한 쪽은 현위치이므로 이쪽을 "기본 자리로"를 뜻하는 집 모양으로
   * 옮겼다. 예전에는 둘 다 눈금 크로스헤어여서 그 상태에서 구분되지 않았다.
   */
  recenter: (
    <>
      <path d="M3.5 9.5 10 4l6.5 5.5" />
      <path d="M5.5 8.5v7h9v-7" />
    </>
  ),
  route: (
    <>
