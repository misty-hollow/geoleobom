...
48비트에서 충돌 확률은 대략 `n^2 / 2^49`다. 실데이터 충청권 123,963행이면
약 2.7×10^-5, 전국으로 넓혀 200만 행이면 약 0.7%다. 작지만 0이 아니므로
`assign_fids`가 충돌을 **찾으면 멈춘다.** 조용히 덮어쓰지 않는다.

**전국 전환 때는 `FID_BITS`를 올려야 할 수 있다.** 올리면 모든 fid가 바뀌므로
수정표의 열쇠가 `(source, source_id)`여야 하는 이유가 하나 더 생긴다.
"""
