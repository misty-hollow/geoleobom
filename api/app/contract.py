...
# 확인할 때 쓰는 여유. OSRM은 좌표를 1e-6도(위도로 약 0.11m) 고정소수점으로 들고 있으므로
# **그 눈금 단위의 정수로 비교한다.** 여유는 한 눈금이며 넓히면 확인이 무의미해진다.
#
# 도(度) 실수로 빼서 1e-6과 견주면 안 된다. 실제 OSRM 응답에서 재현했다 —
# `/table`이 127.120809, `/route`가 127.120810을 돌려준 **한 눈금 차이**인데
# `abs(127.120809 - 127.12081)`은 1.0000000116860974e-06이라 `<= 1e-6`이 거짓이 된다.
# 이진 부동소수점 표현 오차이며 좌표에 따라 참이 되기도 해서, 예전 단위 검사가
# 쓰던 (127.140000, 127.140001) 쌍은 우연히 통과했다.
OSRM_COORD_SCALE: Final[int] = 1_000_000
ROUTE_SNAP_EPSILON_TICKS: Final[int] = 1
